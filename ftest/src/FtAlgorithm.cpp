...
TEST_F(FtAlgorithm, lfu_cache_algo_test)
{
    LFUCache cache(2);
    ...
    ASSERT_EQ(cache.get(1), 1);          // 返回  1
    cache.put(3, 3);                     // 该操作会使得key[2]作废
    ASSERT_EQ(cache.get(2), -1);           // 返回无效值 (未找到)
    cache.put(4, 4);                     // 该操作会使得key[1]作废
    ASSERT_EQ(cache.get(1), 1);                    // 返回无效值 (未找到)
    ASSERT_EQ(cache.get(3), -1);                    // 返回  3
    ASSERT_EQ(cache.get(4), 4);                    // 返回  4
}
